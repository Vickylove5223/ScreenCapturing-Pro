...
export const Timeline: React.FC<TimelineProps> = ({
  ...
  const trackWidth = duration * timelineZoom;
  ...
  const handleTimelineClick = (e: React.MouseEvent) => {
    ...
    const time = Math.max(0, Math.min(duration, x / timelineZoom));
    onSeek(time);
  };
  ...
  const onHandleDrag = (e: React.MouseEvent, type: 'start' | 'end', segment: VideoSegment) => {
    ...
    const onMouseMove = (moveEvent: MouseEvent) => {
      ...
      if (type === 'start') {
        ...
        const newEnd = Math.max(initialStart + 0.1, Math.min(duration, initialEnd + deltaTime));
      ...
      <div className="flex items-center justify-between px-4 h-12 bg-[#3A5A40] border-b border-[#588157]">
        <div className="flex items-center gap-4">
          ...
          <div className="flex items-center gap-2 text-sm font-mono text-[#DAD7CD] font-semibold">
            ...
            <span className="text-[#A3B18A]">{formatTime(duration)}</span>
          </div>
        </div>
