...
import { 
  ...
  Plus, Play, Pause, Monitor, Type, Sparkles, Music, History, Box, Wand2,
  ChevronLeft, ChevronRight, Gauge, ImageIcon, Upload, X, Layers,
...
const PRESET_COLORS = ['#000000', '#344E41', '#ffffff', '#588157', '#A3B18A', '#ef4444', '#10b981'];

const DEFAULT_BG_IMAGES = [
  { id: 'gradient-1', name: 'Magic Purple', url: 'https://images.unsplash.com/photo-1557683316-973673baf926?auto=format&fit=crop&w=400&q=80' },
  { id: 'gradient-2', name: 'Dark Flow', url: 'https://images.unsplash.com/photo-1618005182384-a83a8bd57fbe?auto=format&fit=crop&w=400&q=80' },
  { id: 'abstract-1', name: 'Deep Space', url: 'https://images.unsplash.com/photo-1464802686167-b939a6910659?auto=format&fit=crop&w=400&q=80' },
  { id: 'abstract-2', name: 'Pink Dream', url: 'https://images.unsplash.com/photo-1550684848-fac1c5b4e853?auto=format&fit=crop&w=400&q=80' },
];
...
export const Editor: React.FC<EditorProps> = ({ blob, onBack, onSave }) => {
  ...
  const bgImageSrc = useMemo(() => {
    if (currentState.bgImageBlob) return URL.createObjectURL(currentState.bgImageBlob);
    if (currentState.bgImageUrl) return currentState.bgImageUrl;
    return null;
  }, [currentState.bgImageBlob, currentState.bgImageUrl]);
  ...
  const isBackgroundActive = useMemo(() => {
    return currentState.bgMode === 'image' || (currentState.bgMode === 'color' && currentState.bgColor !== '#000000');
  }, [currentState.bgMode, currentState.bgColor, currentState.bgImageBlob, currentState.bgImageUrl]);
  ...
  const handleExport = async (options: ExportOptions) => {
    ...
    setProgress(0);
    setRenderingText('Initializing Engine...');
    ...
    try {
      ...
      if (videoRef.current) {
        ...
        if (isBackgroundActive) {
            const padding = 50; 
            const canvasW = vidW + (padding * 2);
            const canvasH = vidH + (padding * 2);
            
            layout = {
                canvasSize: { width: canvasW, height: canvasH },
                sourceRect: sourceRect, 
                destRect: { x: padding, y: padding, width: vidW, height: vidH } 
            };
        } else {
            layout = {
      ...
      const resultBlob = await processVideo({
        ...
        backgroundColor: currentState.bgMode === 'color' ? currentState.bgColor : undefined,
        backgroundBlob: currentState.bgImageBlob || undefined,
        backgroundImageUrl: currentState.bgImageUrl || undefined,
        addedAudioBlob: currentState.addedAudioBlob || undefined,
        addedAudioUrl: currentState.addedAudioUrl || undefined,
        format: options.format,
        targetResolution: targetRes,
        layout,
        hasAudioTrack: videoRef.current ? (videoRef.current as any).mozHasAudio || (videoRef.current as any).webkitAudioDecodedByteCount > 0 || (videoRef.current.audioTracks && videoRef.current.audioTracks.length > 0) || true : true,
        onProgress: (p) => {
          ...
          <div className="flex-1 relative flex items-center justify-center p-12 overflow-hidden">
             ...
             <div 
              ...
              style={{
                width: '100%',
                maxWidth: '1200px',
                backgroundColor: currentState.bgMode === 'color' ? currentState.bgColor : '#000',
                backgroundImage: currentState.bgMode === 'image' && bgImageSrc ? `url(${bgImageSrc})` : 'none',
                backgroundSize: 'cover',
                backgroundPosition: 'center',
                padding: isBackgroundActive ? '50px' : '0'
              }}
             >
             ...
             <div className="absolute top-8 right-8 flex flex-col gap-2 bg-[#3A5A40]/90 backdrop-blur rounded-xl p-2 border border-[#588157] shadow-xl z-20">
                 <button onClick={() => updateState({ zoom: Math.min(3, currentState.zoom + 0.1) })} className="p-2 hover:bg-[#588157] rounded text-[#DAD7CD]"><ZoomIn size={16} /></button>
                 <span className="text-[10px] text-center font-bold text-[#A3B18A]">{Math.round(currentState.zoom * 100)}%</span>
                 <button onClick={() => updateState({ zoom: Math.max(1, currentState.zoom - 0.1) })} className="p-2 hover:bg-[#588157] rounded text-[#DAD7CD]"><ZoomIn size={16} className="rotate-180" /></button>
                 {currentState.zoom > 1 && (
                     <button onClick={() => updateState({ zoom: 1, panX: 0, panY: 0 })} className="text-[10px] text-rose-400 font-bold mt-1">RESET</button>
                 )}
             </div>
          </div>
        ...
        <aside className="w-80 bg-[#3A5A40] border-l border-[#588157] p-6 space-y-8 overflow-y-auto custom-scrollbar">
           <div>
              <h3 className="text-[10px] font-black uppercase text-[#A3B18A] mb-4 flex items-center gap-2 tracking-tighter">
                 <Wand2 size={12} className="text-[#DAD7CD]" /> Canvas & Background
              </h3>
              <div className="space-y-4">
                 <div>
                    <div className="flex items-center justify-between mb-2">
                       <label className="text-[11px] text-[#A3B18A] font-medium">BG Color</label>
                       {currentState.bgMode === 'color' && <span className="text-[9px] text-[#DAD7CD] font-bold uppercase">Active</span>}
                    </div>
                    <div className="flex flex-wrap gap-2">
                       {PRESET_COLORS.map(color => (
                          <button 
                            key={color}
                            onClick={() => updateState({ bgMode: 'color', bgColor: color, bgImageBlob: null, bgImageUrl: null, bgImageName: null })}
                            className={`w-6 h-6 rounded-full border transition-transform ${currentState.bgMode === 'color' && currentState.bgColor === color ? 'border-white scale-110 shadow-lg shadow-white/10' : 'border-[#588157]'}`}
                       ...
                       <input 
                         ...
                         onChange={(e) => updateState({ bgMode: 'color', bgColor: e.target.value, bgImageBlob: null, bgImageUrl: null, bgImageName: null })}
                 ...
                 <div className="space-y-3">
                    <div className="flex items-center justify-between">
                       <label className="text-[11px] text-[#A3B18A] font-medium">Library Backgrounds</label>
                       <button 
                        onClick={() => bgImageInputRef.current?.click()}
                        className="text-[10px] text-[#DAD7CD] hover:underline font-bold"
                       >
                         Upload Custom
                       </button>
                    </div>
                    <div className="grid grid-cols-2 gap-2">
                       {DEFAULT_BG_IMAGES.map(img => (
                          <button 
                            key={img.id}
                            onClick={() => updateState({ bgMode: 'image', bgImageUrl: img.url, bgImageBlob: null, bgImageName: img.name })}
                            className={`relative h-12 rounded overflow-hidden border-2 transition-all ${currentState.bgImageUrl === img.url ? 'border-[#DAD7CD] scale-[0.98]' : 'border-transparent opacity-60 hover:opacity-100'}`}
                          >
                 ...
                 {currentState.bgImageName && (
                    <div className="flex items-center justify-between p-2 bg-[#344E41] rounded border border-[#588157]/40">
                       <div className="flex items-center gap-2 overflow-hidden">
                          <ImageIcon size={14} className="text-[#DAD7CD] flex-shrink-0" />
                          <span className="text-[10px] truncate text-[#A3B18A]">{currentState.bgImageName}</span>
                       </div>
                       <button onClick={() => updateState({ bgImageBlob: null, bgImageUrl: null, bgImageName: null, bgMode: 'color', bgColor: '#000000' })} className="p-1 hover:text-white text-[#A3B18A]"><X size={14}/></button>
                    </div>
                 )}
              </div>
           </div>
        ...
        <div className="fixed inset-0 bg-[#344E41]/95 z-[999] flex flex-col items-center justify-center backdrop-blur-lg animate-in fade-in duration-300">
           ...
           {(processingState === 'rendering' || processingState === 'authenticating' || processingState === 'uploading') && (
               <div className="flex flex-col items-center w-full max-w-sm">
                   <div className="w-full h-[2px] bg-[#588157] rounded-full overflow-hidden mb-8">
                      <div className="h-full bg-gradient-to-r from-[#DAD7CD] to-[#A3B18A] transition-all duration-300 ease-out" style={{ width: `${progress}%` }} />
                   </div>
                   <div className="text-[#DAD7CD] font-black text-2xl tracking-[0.1em] uppercase italic animate-pulse">
                        {processingState === 'rendering' ? 'Building Vibe' : (processingState === 'authenticating' ? 'Signing In' : 'Syncing Cloud')}
                   </div>
                   <p className="text-[#A3B18A] text-sm mt-4 font-mono font-bold tracking-widest">{renderingText} {progress > 0 ? progress + '%' : ''}</p>
               </div>
           )}
